...
from vllm_fl.dispatch.backends.vendor.ascend.utils import vllm_version_is
...
# yapf: enable
from vllm_fl.dispatch.backends.vendor.ascend.ascend_config import get_ascend_config
from vllm_fl.dispatch.backends.vendor.ascend.attention.attention_v1 import AscendAttentionBackend, AscendAttentionState
from vllm_fl.dispatch.backends.vendor.ascend.attention.context_parallel.dsa_cp import AscendDSACPMetadataBuilder
from vllm_fl.dispatch.backends.vendor.ascend.attention.dsa_v1 import AscendDSAMetadataBuilder
from vllm_fl.dispatch.backends.vendor.ascend.attention.mla_v1 import AscendMLABackend
from vllm_fl.dispatch.backends.vendor.ascend.attention.utils import AscendCommonAttentionMetadata, using_paged_attention

# yapf conflicts with isort for this block
# yapf: disable
from vllm_fl.dispatch.backends.vendor.ascend.compilation.acl_graph import (
    ACLGraphWrapper,
    set_draft_graph_params,
    set_graph_params,
    update_full_graph_params,
)
from vllm_fl.dispatch.backends.vendor.ascend.eplb.adaptor.vllm_adaptor import VllmEplbAdaptor
from vllm_fl.dispatch.backends.vendor.ascend.eplb.core.eplb_device_transfer_loader import D2DExpertWeightLoader
from vllm_fl.dispatch.backends.vendor.ascend.eplb.core.eplb_worker import EplbProcess
from vllm_fl.dispatch.backends.vendor.ascend.eplb.eplb_updator import EplbUpdator
from vllm_fl.dispatch.backends.vendor.ascend.eplb.utils import model_register
from vllm_fl.dispatch.backends.vendor.ascend.ops.rotary_embedding import set_cos_and_sin, update_cos_sin
from vllm_fl.dispatch.backends.vendor.ascend.patch.worker.patch_draft_quarot import patch_load_weights
from vllm_fl.dispatch.backends.vendor.ascend.quantization.utils import enable_fa_quant
from vllm_fl.dispatch.backends.vendor.ascend.sample.sampler import AscendSampler
from vllm_fl.dispatch.backends.vendor.ascend.spec_decode import get_spec_decode_method
from vllm_fl.dispatch.backends.vendor.ascend.spec_decode.dflash_proposer import AscendDflashProposer
from vllm_fl.dispatch.backends.vendor.ascend.spec_decode.draft_proposer import AscendDraftModelProposer
from vllm_fl.dispatch.backends.vendor.ascend.spec_decode.eagle_proposer import AscendEagleProposer
from vllm_fl.dispatch.backends.vendor.ascend.spec_decode.extract_hidden_states_proposer import (
    AscendExtractHiddenStatesProposer,
)
from vllm_fl.dispatch.backends.vendor.ascend.spec_decode.medusa_proposer import AscendMedusaProposer
from vllm_fl.dispatch.backends.vendor.ascend.spec_decode.ngram_proposer import AscendNgramProposer
from vllm_fl.dispatch.backends.vendor.ascend.spec_decode.ngram_proposer_npu import AscendNgramProposerNPU
from vllm_fl.dispatch.backends.vendor.ascend.spec_decode.suffix_proposer import AscendSuffixDecodingProposer
from vllm_fl.dispatch.backends.vendor.ascend.spec_decode.utils import update_num_computed_tokens_for_batch_change
from vllm_fl.dispatch.backends.vendor.ascend.utils import (
    calc_split_factor,
    check_gdn_layer,
    enable_sp,
    enable_sp_by_pass,
...
from vllm_fl.dispatch.backends.vendor.ascend.worker.npu_input_batch import NPUInputBatch
from vllm_fl.dispatch.backends.vendor.ascend.worker.pcp_utils import PCPManager

from vllm_fl.dispatch.backends.vendor.ascend.ascend_forward_context import (  # isort: skip
    MoECommType,
...
from vllm_fl.dispatch.backends.vendor.ascend.sample.rejection_sampler import AscendRejectionSampler
...
class NPUModelRunner(GPUModelRunner):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        if self.enable_hamming_sparse is True:
            from vllm_fl.dispatch.backends.vendor.ascend.worker.kvcomp_utils import initialize_kvcomp_metadata
    ...
    def execute_model(
        ...
        with record_function_or_nullcontext("prepare input"):
            with self.synchronize_input_prep():
                ...
                # '_update_states_after_model_execute', which is not overridden in vllm-plugin-FL.
        ...
        for kv_cache_gid, kv_cache_group in enumerate(self.kv_cache_config.kv_cache_groups):
            ...
            if self.speculative_config and spec_decode_common_attn_metadata is None:
                if isinstance(self.drafter, AscendEagleProposer | AscendDraftModelProposer | AscendDflashProposer):
                    ...
                    spec_decode_common_attn_metadata = cm
            if self.enable_hamming_sparse is True:
                from vllm_fl.dispatch.backends.vendor.ascend.attention.kvcomp_attn.attention_utils import build_kvcomp_metadata
    ...
    def _dummy_run(
        ...
        if create_mixed_batch:
            raise NotImplementedError("create_mixed_batch is used for warmup deepgemm, vllm-plugin-FL does not need it")
        elif uniform_decode:
        ...
        # vllm-plugin-FL does not support ubatch now
        ...
        if self._should_build_dummy_attn_metadata(force_attention, is_profile, cudagraph_runtime_mode):
            if create_mixed_batch:
                raise NotImplementedError(
                    "create_mixed_batch is used for warmup deepgemm, vllm-plugin-FL does not need it"
                )
    ...
    def initialize_kv_cache_tensors(self, kv_cache_config: KVCacheConfig) -> dict[str, torch.Tensor]:
        ...
        if self.model_config.hf_text_config.model_type == "deepseek_v4":
            from vllm_fl.dispatch.backends.vendor.ascend.utils import extract_dsv4_layer_index
        ...
        if self.enable_hamming_sparse is True:
            from vllm_fl.dispatch.backends.vendor.ascend.worker.kvcomp_utils import init_and_bind_hashk_cache
