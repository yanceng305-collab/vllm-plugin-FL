...
#             `vllm_fl.dispatch.backends.vendor.ascend.utils.adapt_patch(is_global_patch=True)` in
#             `vllm_fl.dispatch.backends.vendor.ascend.platform.NPUPlatform.pre_register_and_update()` function.
# - worker: contains the patches applied when worker starts. It's called by
#           `vllm_fl.dispatch.backends.vendor.ascend.utils.adapt_patch(is_global_patch=False)` in
#           each worker's `__init__` function.
#
# Once a new patch is added in vllm-plugin-FL, please add the patch description into this file as well.
# ----------------------------------------------------------------------------------
...
#       set environmental variable VLLM_FL_BALANCE_SCHEDULING=1 (deprecated).
#    Related PR (if no, explain why):
...
#       vllm-plugin-FL needs to disable fp8 quantization and load bf16 dequantized
...
#       https://github.com/vllm-project/vllm-plugin-FL/issues/8327
#       https://github.com/vllm-project/vllm-plugin-FL/pull/8178
#    Future Plan:
...
#       implementation.
#    Related PR (if no, explain why):
#       No, vllm-plugin-FL-specific profiling / scheduling integration.
#    Future Plan:
...
#       https://github.com/vllm-project/vllm-plugin-FL/pull/8400
#    Future Plan:
...
#    Future Plan:
#       Remove this patch once vllm-plugin-FL upgrades to a vLLM version with the
...
#    Future Plan:
#       Remove this patch when https://github.com/vllm-project/vllm-plugin-FL/issues/5712 is completed.
...
#    Future Plan:
#       Remove this patch when the PTA version used by vllm-plugin-FL has been upgraded.
#
...
#    Why:
#       vllm-plugin-FL reused the loading logic of drafter model from vllm,
...
#    Why:
#       When GLM5 uses rotary quant in vllm-plugin-FL, the MTP layer needs to load an extra weight
#       named `rot.weight`.
#    How：
#       If weight name starts with `rot`, return `layer_id + i` like other tensors in MTP layer.
#    Related PR (if no, explain why):
#       Rotary quant is a unique feature of vllm-plugin-FL.
#    Future Plan:
...
#    Why:
#       When GLM5 uses rotary quant in vllm-plugin-FL, the `previous_hidden_states` does not .
#    How：
...
#    Future Plan:
#       remove this patch when vllm-plugin-FL's make_dummy behavior aligns with vLLM.
# ** 23. File: worker/patch_v2/patch_block_table.py**
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#   1. `vllm.v1.worker.gpu.block_table.BlockTables`
#    Why:
##      vllm-plugin-FL need to initialize slot mapping as torch.int32 dtype,
...
#    Future Plan:
#       remove this patch when vllm-plugin-FL's BlockTables can initialize
...
#    Future Plan:
#       remove this when vllm-plugin-FL's attention metadata is align with vllm.
...
#       when using mrope.
#    Future Plan:
#       Remove this patch when vllm-plugin-FL supports pattern matching for this fused kernel.
...
#    How：
#       Replace ops.* with the internal implementation of vllm-plugin-FL.
#    Future Plan:
#       Remove this patch when vllm-plugin-FL supports pattern matching for ops.*.
