...
from vllm_fl.dispatch.backends.vendor.ascend.utils import COMPRESSED_TENSORS_METHOD
...
class AscendCompressedTensorsConfig(QuantizationConfig):
    ...
    def get_quant_method(
        ...
        if isinstance(layer, LinearBase):
            ...
            logger.info_once("Using the vllm-plugin-FL llmcompressor Quantization now!")
            return AscendLinearMethod(linear_scheme)

        if isinstance(layer, FusedMoE):
            # Delayed import to avoid circular import
            from vllm_fl.dispatch.backends.vendor.ascend.ops.fused_moe.fused_moe import AscendUnquantizedFusedMoEMethod
