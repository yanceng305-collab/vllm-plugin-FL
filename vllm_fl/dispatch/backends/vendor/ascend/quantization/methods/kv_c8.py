...
def _fa_quant_weight_loader(param: torch.Tensor, loaded_weight: torch.Tensor):
    ...
    if param.numel() == 1 and loaded_weight.numel() == 1:
        ...
        assert param.size() == loaded_weight.size(), (
            "[vllm-plugin-FL/FAKQuant] Attempted to load weight "
...
class AscendC8KVCacheAttentionMethod(AscendAttentionScheme):
    ...
    def create_weights(self, layer: torch.nn.Module) -> None:
        ...
        if not self.is_kv_producer:
            logger.info_once(
                "[vllm-plugin-FL/C8_KV] KV cache producer is disabled; setting kv_cache_torch_dtype to torch.int8."
            )
            layer.kv_cache_torch_dtype = torch.int8
        # Upgrade impl to the C8-specific subclass so the C8 forward path is always used.
        if hasattr(layer, "impl"):
            from vllm_fl.dispatch.backends.vendor.ascend.attention.attention_v1 import AscendC8AttentionBackendImpl
    ...
    def apply(
        ...
        err_msg = (
            "[vllm-plugin-FL/C8_KV] AscendC8KVCacheAttentionMethod.apply should "
