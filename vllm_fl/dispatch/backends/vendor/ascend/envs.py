...
env_variables: dict[str, Callable[[], Any]] = {
    ...
    # If set, vllm-plugin-FL will print verbose logs during compilation
    ...
    "VLLM_FL_ENABLE_MATMUL_ALLREDUCE": lambda: bool(int(os.getenv("VLLM_FL_ENABLE_MATMUL_ALLREDUCE", "0"))),
    # Whether to enable FlashComm optimization when tensor parallel is enabled.
    # This feature will get better performance when concurrency is large.
    # DEPRECATED: use additional_config.enable_flashcomm1 instead.
    "VLLM_FL_ENABLE_FLASHCOMM1": lambda: bool(int(os.getenv("VLLM_FL_ENABLE_FLASHCOMM1", "0"))),
    ...
    "VLLM_FL_FLASHCOMM2_PARALLEL_SIZE": lambda: int(os.getenv("VLLM_FL_FLASHCOMM2_PARALLEL_SIZE", 0)),
    # Whether to enable msMonitor tool to monitor the performance of vllm-plugin-FL.
    ...
    "VLLM_FL_ENABLE_MLAPO": lambda: bool(int(os.getenv("VLLM_FL_ENABLE_MLAPO", "1"))),
    # Whether to enable weight cast format to FRACTAL_NZ.
    # 0: close nz;
    # 1: only quant case enable nz;
    # 2: enable nz as long as possible.
    "VLLM_FL_ENABLE_NZ": lambda: int(os.getenv("VLLM_FL_ENABLE_NZ", 1)),
    # Decide whether we should enable CP parallelism.
    "VLLM_FL_ENABLE_CONTEXT_PARALLEL": lambda: bool(int(os.getenv("VLLM_FL_ENABLE_CONTEXT_PARALLEL", "0"))),
    ...
    "VLLM_FL_ENABLE_FUSED_MC2": lambda: int(os.getenv("VLLM_FL_ENABLE_FUSED_MC2", "0")),
    # DEPRECATED: VLLM_FL_BALANCE_SCHEDULING env var will be removed in a future release.
    # Use --additional-config '{"enable_balance_scheduling": true}' instead.
    "VLLM_FL_BALANCE_SCHEDULING": lambda: bool(int(os.getenv("VLLM_FL_BALANCE_SCHEDULING", "0"))),
    # use fused op transpose_kv_cache_by_block, default is True
    "VLLM_FL_FUSION_OP_TRANSPOSE_KV_CACHE_BY_BLOCK": lambda: bool(
        int(os.getenv("VLLM_FL_FUSION_OP_TRANSPOSE_KV_CACHE_BY_BLOCK", "1"))
    ),
    # Control the aclrtMemcpyBatchAsync compile path for KV cache offloading.
    # "1": force enable, "0": force disable, None: auto-detect from CANN headers.
    "VLLM_FL_ENABLE_BATCH_MEMCPY": lambda: os.getenv("VLLM_FL_ENABLE_BATCH_MEMCPY", None),
    # Whether to use MultiBlockPool for KV cache management
    "VLLM_FL_APPLY_DSV4_PATCH": lambda: bool(int(os.getenv("VLLM_FL_APPLY_DSV4_PATCH", "0"))),
}
