...
from vllm_fl.dispatch.backends.vendor.ascend.ops.triton.mul_add import muls_add_triton
...
class DeepseekV2DecoderLayer(nn.Module):
    ...
    def hc_pre(self, x: torch.Tensor, hc_fn: torch.Tensor, hc_scale: torch.Tensor, hc_base: torch.Tensor):
        # HC ops are the first native (_C_ascend) kernels used in the decoder
        # forward; ensure the Ascend kernels are loaded in this process before
        # use (idempotent / cached). Mirrors the enable_custom_op() gating used
        # by the layernorm / fused_moe custom ops.
        enable_custom_op()
...
class DeepseekV4Model(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # Load (register) the native Ascend kernels eagerly, at model
        # construction time (which runs before torch.compile / graph capture).
        # This sets the ``_CUSTOM_OP_ENABLED`` guard so that the
        # ``enable_custom_op()`` calls inside the traced forward (e.g. hc_pre)
        # short-circuit during graph capture instead of triggering the kernel
        # load (which is not Dynamo-traceable).
        enable_custom_op()
    ...
    def forward(
        ...
        from vllm_fl.dispatch.backends.vendor.ascend.ascend_forward_context import get_forward_context
