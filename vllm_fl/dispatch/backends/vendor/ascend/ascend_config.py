...
class AscendConfig:
    ...
    def __init__(self, vllm_config: "VllmConfig"):
        ...
        from vllm_fl.dispatch.backends.vendor.ascend import envs as ascend_envs

        self.enable_balance_scheduling = self._get_config_value(
            additional_config,
            "enable_balance_scheduling",
            "VLLM_FL_BALANCE_SCHEDULING",
            ascend_envs.VLLM_FL_BALANCE_SCHEDULING,
        )
        self.enable_flashcomm1 = self._get_config_value(
            additional_config,
            "enable_flashcomm1",
            "VLLM_FL_ENABLE_FLASHCOMM1",
            ascend_envs.VLLM_FL_ENABLE_FLASHCOMM1,
        )
        ...
        from vllm_fl.dispatch.backends.vendor.ascend.utils import enable_sp
        ...
        self.enable_context_parallel = self._get_config_value(
            ...
            "VLLM_FL_ENABLE_CONTEXT_PARALLEL",
            ascend_envs.VLLM_FL_ENABLE_CONTEXT_PARALLEL,
        )
        self.enable_matmul_allreduce = self._get_config_value(
            additional_config,
            "enable_matmul_allreduce",
            "VLLM_FL_ENABLE_MATMUL_ALLREDUCE",
            ascend_envs.VLLM_FL_ENABLE_MATMUL_ALLREDUCE,
        )
        self.enable_fused_mc2 = self._get_config_value(
            additional_config,
            "enable_fused_mc2",
            "VLLM_FL_ENABLE_FUSED_MC2",
            ascend_envs.VLLM_FL_ENABLE_FUSED_MC2,
        )
        self.enable_mlapo = self._get_config_value(
            additional_config,
            "enable_mlapo",
            "VLLM_FL_ENABLE_MLAPO",
            ascend_envs.VLLM_FL_ENABLE_MLAPO,
        )
        self.enable_flashcomm2_parallel_size = self._get_config_value(
            additional_config,
            "enable_flashcomm2_parallel_size",
            "VLLM_FL_FLASHCOMM2_PARALLEL_SIZE",
            ascend_envs.VLLM_FL_FLASHCOMM2_PARALLEL_SIZE,
        )
        ...
        self.enable_transpose_kv_cache_by_block = self._get_config_value(
            ...
            "VLLM_FL_FUSION_OP_TRANSPOSE_KV_CACHE_BY_BLOCK",
            ascend_envs.VLLM_FL_FUSION_OP_TRANSPOSE_KV_CACHE_BY_BLOCK,
        )
        ...
        from vllm_fl.dispatch.backends.vendor.ascend.utils import get_flashcomm2_config_and_validate
        ...
        from vllm_fl.dispatch.backends.vendor.ascend.utils import AscendDeviceType, get_ascend_device_type
    ...
    def _materialize_dump_config_to_file(dump_config: dict[str, Any]) -> str:
        dump_config_dir = os.path.join(os.getcwd(), ".vllm_fl.dispatch.backends.vendor.ascend", "msprobe")
    ...
    def update_compile_ranges_split_points(self):
        ...
        if self.ascend_compilation_config.enable_npugraph_ex:
            if self.ascend_compilation_config.fuse_allreduce_rms:
                from vllm_fl.dispatch.backends.vendor.ascend.compilation.passes.allreduce_rmsnorm_fusion_pass import ALLREDUCE_NORM_FUSE_THRESHOLD
...
def clear_ascend_config():
    global _ASCEND_CONFIG
    _ASCEND_CONFIG = None
    from vllm_fl.dispatch.backends.vendor.ascend.utils import clear_enable_sp
