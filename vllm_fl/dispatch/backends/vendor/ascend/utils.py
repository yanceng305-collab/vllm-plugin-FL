...
import vllm_fl.dispatch.backends.vendor.ascend.envs as envs_ascend
from vllm_fl.dispatch.backends.vendor.ascend.ascend_config import WeightPrefetchConfig, get_ascend_config
...
def device_print(
    ...
    Supported usage:
        ...
        >>> from vllm_fl.dispatch.backends.vendor.ascend.utils import device_print
        >>> device_print(x)
...
# - non-310P: follow VLLM_FL_ENABLE_NZ
# - FP32: never convert
...
def enable_custom_op():
    """
    Enable lazy init for fl_ascend_C to avoid early initialization of CANN's RTS component.
    ...
    # with batch invariant in vllm-plugin-FL, we need to disable them.
    ...
    # https://github.com/vllm-project/vllm-plugin-FL/issues/7157 for latest update about custom op.
    ...
    try:
        ...
        from vllm_fl.dispatch.backends.vendor.ascend.kernels import load_ascend_kernels as _load_ascend_kernels; _load_ascend_kernels()

        # register the meta implementation for custom kernel if necessary
        import vllm_fl.dispatch.backends.vendor.ascend.meta_registration  # type: ignore  # noqa: F401
...
def set_weight_prefetch_method(weight_prefetch_config: WeightPrefetchConfig):
    ...
    if _WEIGHT_PREFETCH_METHOD is None:
        from vllm_fl.dispatch.backends.vendor.ascend.ops.weight_prefetch import WeightPrefetchMethod
...
def adapt_patch(is_global_patch: bool = False):
    if is_global_patch:
        from vllm_fl.dispatch.backends.vendor.ascend.patch import platform  # noqa: F401
    else:
        from vllm_fl.dispatch.backends.vendor.ascend.patch import worker  # noqa: F401
...
def register_ascend_customop(vllm_config: VllmConfig | None = None):
    ...
    from vllm_fl.dispatch.backends.vendor.ascend.ops.activation import AscendQuickGELU, AscendSiluAndMul
    from vllm_fl.dispatch.backends.vendor.ascend.ops.bailing_moe_linear_attn import AscendBailingMoELinearAttention
    from vllm_fl.dispatch.backends.vendor.ascend.ops.conv import AscendConv3dLayer
    from vllm_fl.dispatch.backends.vendor.ascend.ops.fused_moe.fused_moe import AscendFusedMoE
    from vllm_fl.dispatch.backends.vendor.ascend.ops.gdn import AscendGatedDeltaNetAttention
    from vllm_fl.dispatch.backends.vendor.ascend.ops.layernorm import AscendGemmaRMSNorm, AscendRMSNorm, AscendRMSNormGated
    from vllm_fl.dispatch.backends.vendor.ascend.ops.linear import (
    ...
    from vllm_fl.dispatch.backends.vendor.ascend.ops.mla import AscendMultiHeadLatentAttention
    from vllm_fl.dispatch.backends.vendor.ascend.ops.mm_encoder_attention import AscendMMEncoderAttention
    from vllm_fl.dispatch.backends.vendor.ascend.ops.qwen2_decoder import AscendCustomQwen2Decoder
    from vllm_fl.dispatch.backends.vendor.ascend.ops.rel_pos_attention import AscendRelPosAttention
    from vllm_fl.dispatch.backends.vendor.ascend.ops.rotary_embedding import (
    ...
    from vllm_fl.dispatch.backends.vendor.ascend.ops.vocab_parallel_embedding import (
    ...
    if vllm_config is not None and vllm_config.model_config.is_deepseek_mla:
        from vllm_fl.dispatch.backends.vendor.ascend.ops.fused_moe.gate_linear import AscendGateLinear

        REGISTERED_ASCEND_OPS["GateLinear"] = AscendGateLinear

    # 310P: override selected ops with 310P implementations (keep minimal changes outside _310p)
    if is_310p():
        from vllm_fl.dispatch.backends.vendor.ascend._310p.fused_moe.fused_moe import AscendFusedMoE310
        from vllm_fl.dispatch.backends.vendor.ascend._310p.ops.activation import AscendSiluAndMul310
        from vllm_fl.dispatch.backends.vendor.ascend._310p.ops.conv import AscendConv3dLayer310
        from vllm_fl.dispatch.backends.vendor.ascend._310p.ops.fla.gdn_310 import AscendGatedDeltaNetAttention310
        from vllm_fl.dispatch.backends.vendor.ascend._310p.ops.layernorm import (
            AscendGemmaRMSNorm310,
            AscendRMSNorm310,
            AscendRMSNormGated310,
        )
        from vllm_fl.dispatch.backends.vendor.ascend._310p.ops.mm_encoder_attention import AscendMMEncoderAttention310
        from vllm_fl.dispatch.backends.vendor.ascend._310p.ops.rotary_embedding import AscendMRotaryEmbedding310, AscendRotaryEmbedding310
        from vllm_fl.dispatch.backends.vendor.ascend._310p.ops.vocab_parallel_embedding import (
...
def _init_ascend_device_type():
    ...
    from vllm_fl.dispatch.backends.vendor.ascend import _build_info  # type: ignore
    ...
    if _ENABLE_SP is None or refresh:
        if additional_config is not None and "enable_flashcomm1" in additional_config:
            ...
            try:
                ...
                _ENABLE_SP = envs_ascend.VLLM_FL_ENABLE_FLASHCOMM1
...
def should_skip_allreduce_across_dp_group(vllm_config, is_draft_model: bool = False) -> bool:
    ...
    from vllm_fl.dispatch.backends.vendor.ascend.ascend_forward_context import select_moe_comm_method
    from vllm_fl.dispatch.backends.vendor.ascend.ops.fused_moe.moe_comm_method import MoECommType
    ...
    if compilation_config.cudagraph_capture_sizes:
        ...
        if potential_max_tokens != compilation_config.max_cudagraph_capture_size:
            logger.warning_once(
                ...
                "see the issue #8240(https://github.com/vllm-project/vllm-plugin-FL/issues/8240).",
                compilation_config.max_cudagraph_capture_size,
...
def enable_dsa_cp() -> bool:
    ...
    if dsa_cp_enable and not enable_sp():
        raise ValueError(
            "DSA CP requires SP to be enabled. Please enable SP(set VLLM_FL_ENABLE_FLASHCOMM1=1) to use DSA CP."
        )
